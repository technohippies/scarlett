...
const SwitchControl = <T extends ValidComponent = "div">(
  // Changed default ValidComponent from input to div to match structure
  ...
  return (
    <>
      <SwitchPrimitive.Input
        class={cn(
          // Combined input and control classes for simplicity in this structure
          "peer sr-only", // Hide the actual input, rely on Control for visuals
          ...
          // local.class // Input class might not be needed if hidden
        )}
        {...others} // Pass non-visual props like value, checked, etc. to Input
      />
      {/* Control renders the visual switch */}
      <SwitchPrimitive.Control
        class={cn(
          ...
          // Apply focus styles directly to Control since Input is hidden
          "peer-focus-visible:outline-none peer-focus-visible:ring-2 peer-focus-visible:ring-ring peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-background",
          local.class // Apply user's class to the Control
        )}
        // Don't spread others here, they went to Input
      >
        {local.children} {/* Render children (the Thumb) inside Control */}
      </SwitchPrimitive.Control>
    </>
  )
}
